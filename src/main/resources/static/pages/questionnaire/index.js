            ...
            <div class="list-header">
              ...
              <div>
                ...
                <button type="button" class="btn btn-link" onclick="onDelProject('${item.id}')">删除</button>
                 <button type="button" class="btn btn-link" onclick="seeStats('${item.id}')">统计</button>
              </div>
            </div>
...
const onCreateQuestionnaire = (id) => {
  ...
  if (project) {
    // If project is found, construct the URL with projectId and projectName
    location.href = "/pages/createQuestionnaire/index.html?projectId=" + id + "&projectName=" + project.projectName;
  } else {
...
const seeStats = (id) =>{
  location.href = "/pages/stats/index.html?projectId="+id
}